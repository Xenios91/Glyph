...
def client() -> TestClient:
    ...
    try:
        ...
        pass
    app.include_router(df_router, prefix="/dangerous-functions")
...
class TestGetCatalog:
    ...
    def test_get_full_catalog(self, client: TestClient) -> None:
        """Test retrieving full catalog without category filter."""
        response = client.get("/dangerous-functions/catalog")
    ...
    def test_catalog_entry_structure(self, client: TestClient) -> None:
    ...
    def test_catalog_categories(self, client: TestClient) -> None:
    ...
    def test_get_catalog_by_category(self, client: TestClient) -> None:
        """Test filtering catalog by category."""
        response = client.get("/dangerous-functions/catalog?category=Buffer Overflow")
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
        entries = data["data"]["entries"]
    ...
    def test_get_catalog_by_unknown_category(self, client: TestClient) -> None:
        """Test filtering by unknown category returns empty list."""
        response = client.get("/dangerous-functions/catalog?category=Nonexistent")
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
...
class TestGetCatalogEntry:
    ...
    def test_get_existing_entry(self, client: TestClient) -> None:
        ...
        response = client.get("/dangerous-functions/catalog/strcpy")
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True
    ...
    def test_get_nonexistent_entry(self, client: TestClient) -> None:
        ...
        response = client.get("/dangerous-functions/catalog/safe_function_xyz")
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is False
        assert "not found" in data["error"]["message"].lower()

    def test_get_entry_case_insensitive(self, client: TestClient) -> None:
    ...
    def test_entry_response_structure(self, client: TestClient) -> None:
        """Test entry response has all required fields."""
        response = client.get("/dangerous-functions/catalog/gets")
        data = response.json()
        entry = data["data"]
...
class TestGetAvailableModels:
    ...
    def test_get_available_models(self, client: TestClient) -> None:
        """Test retrieving available models and prediction tasks."""
        set_dependency_override(client, get_current_active_user, mock_current_user)
        ...
        with patch("app.api.v1.endpoints.dangerous_functions.MLPersistanceUtil") as mock_ml, \
            ...
            response = client.get("/dangerous-functions/available-models")
            assert response.status_code == 200
            data = response.json()
            assert data["success"] is True
    ...
    def test_get_available_models_empty(self, client: TestClient) -> None:
    ...
    def test_get_available_models_no_auth(self, client: TestClient) -> None:
        """Test that endpoint requires authentication (mocked dependency raises)."""
        from fastapi import HTTPException
        def raise_unauthenticated() -> None:
            raise HTTPException(status_code=401, detail="Not authenticated")

        set_dependency_override(client, get_current_active_user, raise_unauthenticated)
        response = client.get("/dangerous-functions/available-models")
...
class TestScanEndpoint:
    ...
    def test_scan_missing_target(self, client: TestClient) -> None:
    ...
    def test_scan_model_not_found(self, client: TestClient) -> None:
    ...
    def test_scan_task_not_found(self, client: TestClient) -> None:
    ...
    def test_scan_model_no_functions(self, client: TestClient) -> None:
    ...
    def test_scan_model_with_dangerous_functions(self, client: TestClient) -> None:
        """Test scan detects dangerous functions in model."""
        set_dependency_override(client, get_current_active_user, mock_current_user)

        mock_funcs = [
            self._make_mock_function("strcpy", "0x401000", "char buf[100]; strcpy(buf, src);"),
            self._make_mock_function("gets", "0x402000", "char buf[64]; gets(buf);"),
    ...
    def test_scan_report_structure(self, client: TestClient) -> None:
    ...
    def test_scan_result_structure(self, client: TestClient) -> None:
        """Test individual scan results have correct structure."""
        set_dependency_override(client, get_current_active_user, mock_current_user)

        mock_funcs = [
            self._make_mock_function("strcpy", "0xDEADBEEF", "strcpy(dst, src); return 0;"),
        ]
    ...
    def test_scan_preserves_entrypoint(self, client: TestClient) -> None:
        """Test that memory address is preserved in results."""
        set_dependency_override(client, get_current_active_user, mock_current_user)

        mock_funcs = [
            self._make_mock_function("gets", "0xCAFE0000", "gets(buf);"),
        ]
    ...
    def test_scan_results_sorted_by_severity(self, client: TestClient) -> None:
        """Test that results are sorted by severity (Critical first)."""
        set_dependency_override(client, get_current_active_user, mock_current_user)

        mock_funcs = [
            self._make_mock_function("sprintf", "0x401000", "sprintf(buf, fmt);"),  # High
            self._make_mock_function("gets", "0x402000", "gets(buf);"),  # Critical
        ]
    ...
    def test_scan_by_task_name(self, client: TestClient) -> None:
        ...
        func_data: list[dict[str, str | list[str]]] = [
            {"functionName": "strcpy", "lowAddress": "0x401000", "tokenList": ["strcpy(buf, src);"]},
        ]
    ...
    def test_scan_model_and_task_preference(self, client: TestClient) -> None:
