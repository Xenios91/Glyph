...
class _MockStep(PipelineStep):
    ...
    def __init__(self, name: str, set_error: bool = False, raise_error: bool = False, data_to_set: dict | None = None):
        self._name = name
...
class TestPipelineStep:
    ...
    def test_mock_step_description(self):
...
class TestProcessingPipeline:
    ...
    def test_init(self):
        ...
        pipeline = ProcessingPipeline("test_pipeline", steps)
    ...
    async def test_execute_multiple_steps_success(self):
        ...
        pipeline = ProcessingPipeline("multi", steps)
        ctx = PipelineContext(uuid="test-uuid", binary_path="/test/binary")
        result = await pipeline.execute(ctx)
    ...
    @pytest.mark.asyncio
    async def test_execute_step_sets_error(self):
        ...
        pipeline = ProcessingPipeline("error", steps)
        ctx = PipelineContext(uuid="test-uuid", binary_path="/test/binary")
        result = await pipeline.execute(ctx)
        ...
        assert steps[2]._executed is False

    @pytest.mark.asyncio
    async def test_execute_step_raises_exception(self):
        ...
        pipeline = ProcessingPipeline("exception", steps)
        ctx = PipelineContext(uuid="test-uuid", binary_path="/test/binary")
        result = await pipeline.execute(ctx)
